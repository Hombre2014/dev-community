...
class MembersController < ApplicationController
  ...
  def update_description
    if current_user.update(about: params[:user][:about])
      render_turbo_stream('replace', 'member-description', 'members/member_description', { user: current_user })
    end
  end
  ...
  def update_personal_details
    if current_user.update(user_personal_info_params)
      render_turbo_stream('replace', 'member-personal-details', 'members/member_personal_details', { user: current_user })
    end
  end
  ...
  def user_personal_info_params
    params.require(:user).permit(:first_name, :last_name, :city, :state, :country, :postcode, :street_address, :profile_title)
  end
end
